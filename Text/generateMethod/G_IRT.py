...
###################################
    ...
    def generate_replenish(self, exist_task, generate_file, test_loader):
        ...
        for line in reader[1:]:
            example, worker, label = line.split(',')[1:4]
                ...
                if example in exist_task:
                    ...
                    if label == -1:
                        ...
                        for label_item in self.label_set:
                            select_label = max(self.e2lpd[example], key=self.e2lpd[example].get)
                            if label_item == select_label:
...
if __name__ == '__main__':
    crowd_file = r'../data/text_feature.csv'
    truth_file = r'../data/text_truth.csv'
    exist_task = sampling(crowd_file, 0).run_fixed_task()
    irt = RDG_generate(crowd_file, truth_file)
    generate_file = '../data/Generate_data/IRT_generate_date.csv'
    test_loader = None
    irt.run()
    irt.generate_replenish(exist_task, generate_file, None)
